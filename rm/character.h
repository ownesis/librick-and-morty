 ...
 * \param[in] ctx (rm_character_t *) Address of rm_character_t structure
 *
 * */
void rm_character_clear(rm_character_t *character);

/**
 * \fn void rm_da_character_clear(struct RM_character_array *character)
 * \brief This function free all episode fields and character inside da_array character.
 * \param[in] ctx (rm_character_t *) Address of RM_character_array structure
 *
 * */
 ...
 * \return (rm_character_t*) Address of struct RM_character with field filled.
 *
 * */
rm_character_t *rm_character_get(uint64_t id);

/**
 * \fn struct RM_character_array rm_character_get_all(rm_character_t *filter)
 * \brief This function get all characters and their attributes.
 * \param[in] filter (rm_character_t *) Address of a RM_character structure to use like filter (only the following field are verifed: name, status, species, type, gender). 
...
/**
 * \fn struct RM_character_array rm_character_get_list(rm_ctx_t *ctx, uint64_t array_id[], size_t array_len)
 * \brief This function get all attributes of character with id inside array_id.
 * \param[in] ctx (rm_ctx_t *) Address of rm_ctx_t context structure
